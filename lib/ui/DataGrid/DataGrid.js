...
// Input rows
		if (!!props.rows) {
			this._setRows(0, props.rows);
//			defaultState.cells = this._getCells()
		}
...
console.log('componentWillReceiveProps', nextProps, !!nextProps.rows);
		...
		if (!!nextProps.rows) {
			this._clearRows();
			this._setRows(0, nextProps.rows);
//			this.setState({
...
console.log('_setRow #2');
	...
	setRow(rowIndex, row, opt_skipUpdate) {
	...
	removeRow(rowIndex) {
		...
		this.setState({
			selectedIndex: rowIndex <= this._data_length
		});
//		this.updateRows();
	};
 ...
 */
	_renderTextControl(rowIndex, column, value) {
		...
		return (
			<div style={{width: column.width}} className={control.className}>
				<Input
					value={value}
...
//console.log('_renderRow', rowIndex, rowCells);
console.log('_renderRow', rowIndex);
...
//
		return (
			<div className={classes} onScroll={this._scrollEvent.bind(this)}>
