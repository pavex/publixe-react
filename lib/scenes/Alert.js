 ...
 */
	constructor(props, context) {
		super(props, context);
		this.state = {};
		this.state.title = this.props.title || 'Alert!';
		this.state.message = this.props.message;
		this.state.width = this.props.width || null;
		this.state.buttonTitle = this.props.buttonTitle || 'Ok';
	};
	...
	render() {
		return (
			<Dialog width={this.state.width} title={this.state.title}>
				...
				<Dialog.Actions>
					<Button primary onClick={this._closeClick.bind(this)}>{this.state.buttonTitle}</Button>
				</Dialog.Actions>
			</Dialog>
		);
	};
